...
**Re-Export Layer Inventory**:

1. **`src/orchestration/jobs/tracking/utils/mlflow_utils.py`** (21 lines)
   - **Functions re-exported**: `get_mlflow_run_url`, `retry_with_backoff`
   - **Source module**: `infrastructure.tracking.mlflow.urls`, `infrastructure.tracking.mlflow.utils`
   - **Usages**: 4 total
     - `src/orchestration/jobs/tracking/trackers/benchmark_tracker.py` (line 21) - imports `retry_with_backoff`
     - `src/orchestration/jobs/tracking/trackers/training_tracker.py` (line 21) - imports `retry_with_backoff`
     - `src/orchestration/jobs/tracking/trackers/conversion_tracker.py` (line 21) - imports `retry_with_backoff`
     - `src/orchestration/jobs/tracking/mlflow_utils.py` (line 10) - nested re-export
   - **Status**: ✅ **ACTIVELY USED** (3 tracker files + 1 nested re-export)

2. **`src/orchestration/jobs/tracking/mlflow_utils.py`** (18 lines)
   - **Functions re-exported**: `get_mlflow_run_url`, `retry_with_backoff`
   - **Source module**: `orchestration.jobs.tracking.utils.mlflow_utils` (nested re-export)
   - **Usages**: 0 direct imports
   - **Status**: ❌ **UNUSED** (nested re-export, no direct consumers)

3. **`src/orchestration/mlflow_utils.py`** (21 lines)
   - **Functions re-exported**: `setup_mlflow_for_stage`
   - **Source module**: `infrastructure.tracking.mlflow.setup`
   - **Usages**: 0 imports found
   - **Status**: ❌ **UNUSED** (deprecated, no active consumers)

4. **`src/orchestration/jobs/tracking/mlflow_helpers.py`** (21 lines)
   - **Functions re-exported**: `create_child_run`
   - **Source module**: `infrastructure.tracking.mlflow.runs`
   - **Usages**: 1 total
     - `src/infrastructure/platform/adapters/mlflow_context.py` (line 170) - imports `create_child_run` (with TODO comment to move to tracking.mlflow)
...
5. **`src/orchestration/jobs/tracking/utils/helpers.py`** (18 lines)
   ...
   - **Source module**: `infrastructure.tracking.mlflow.runs`
   - **Usages**: 0 imports found
   - **Status**: ❌ **UNUSED** (duplicate of mlflow_helpers.py, no consumers)

6. **`src/orchestration/jobs/tracking/utils/__init__.py`** (27 lines)
   - **Functions re-exported**: `create_child_run`, `get_mlflow_run_url`, `retry_with_backoff`
   - **Source module**: `infrastructure.tracking.mlflow.runs`, `infrastructure.tracking.mlflow.urls`, `infrastructure.tracking.mlflow.utils`
   - **Usages**: 0 imports found
   - **Status**: ❌ **UNUSED** (convenience exports, no consumers)

**Test Imports**:
- No tests import from MLflow utility re-export layers
- Tests import from `orchestration.naming` and `orchestration` (different modules, not MLflow utilities)

**Notebook Imports**:
- No notebooks import from MLflow utility re-export layers
- Notebooks import from `orchestration.jobs.tracking.mlflow_tracker` (different module, not utility re-exports)

**Summary**:
- **Total re-export layers**: 6 files
- **Actively used**: 2 layers (`utils/mlflow_utils.py`, `mlflow_helpers.py`)
- **Unused**: 4 layers (`mlflow_utils.py` nested, `orchestration/mlflow_utils.py` deprecated, `utils/helpers.py` duplicate, `utils/__init__.py` convenience)
- **Total active usages**: 4 files importing from re-export layers
  - 3 tracker files using `orchestration.jobs.tracking.utils.mlflow_utils.retry_with_backoff`
  - 1 adapter file using `orchestration.jobs.tracking.mlflow_helpers.create_child_run`
...
**Actions Completed**:
1. ✅ Deleted `src/orchestration/jobs/tracking/mlflow_utils.py` (nested re-export, 0 direct imports)
2. ✅ Deleted `src/orchestration/jobs/tracking/mlflow_helpers.py` (duplicate of `utils/helpers.py`)
3. ✅ Updated `src/infrastructure/platform/adapters/mlflow_context.py`:
   - Changed import from `orchestration.jobs.tracking.mlflow_helpers` to `orchestration.jobs.tracking.utils.helpers`
   - Updated comment to indicate backward compat shim
4. ✅ Updated `src/orchestration/jobs/tracking/utils/__init__.py`:
   - Enhanced documentation to clarify it's a backward compatibility layer
   - Documented source modules for each re-exported function

**Verification**:
- ✅ No remaining references to deleted files (grep confirmed)
- ✅ All functions still accessible through remaining re-export layers:
  - `create_child_run` available via `utils/helpers.py` and `utils/__init__.py`
  - `get_mlflow_run_url` and `retry_with_backoff` available via `utils/mlflow_utils.py` and `utils/__init__.py`
- ✅ No linter errors introduced

**Remaining Re-Export Layers** (after consolidation):
- `orchestration.jobs.tracking.utils.mlflow_utils` - Used by 3 tracker files
- `orchestration.jobs.tracking.utils.helpers` - Used by 1 adapter file
- `orchestration.jobs.tracking.utils.__init__.py` - Convenience exports (unused but kept for backward compat)
...
**Actions Completed**:
1. ✅ Updated `src/orchestration/__init__.py`:
   - Removed fallback import from `.mlflow_utils`
   - Changed to direct import from `infrastructure.tracking.mlflow.setup`
   - Removed try/except fallback logic
2. ✅ Deleted `src/orchestration/mlflow_utils.py` (deprecated module with deprecation warning)
3. ✅ Verified no remaining references:
   - No imports from `orchestration.mlflow_utils` in src/, tests/, or notebooks/
   - Only remaining `mlflow_utils` references are to `orchestration.jobs.tracking.utils.mlflow_utils` (different module, kept for backward compat)

**Verification**:
- ✅ File deleted (glob search confirmed)
- ✅ No broken imports (grep shows only references to different `utils.mlflow_utils` module)
- ✅ No linter errors
- ✅ Deprecation warning removed (module no longer exists)

**Impact**:
- Removed 21 lines of deprecated re-export code
- Eliminated deprecation warning from codebase
- Simplified import path in `orchestration/__init__.py`
...
**Actions Completed**:
1. ✅ Updated `src/orchestration/jobs/tracking/trackers/benchmark_tracker.py`:
   - Changed `from orchestration.jobs.tracking.utils.mlflow_utils import retry_with_backoff`
   - To: `from infrastructure.tracking.mlflow.utils import retry_with_backoff`

2. ✅ Updated `src/orchestration/jobs/tracking/trackers/training_tracker.py`:
   - Changed `from orchestration.jobs.tracking.utils.mlflow_utils import retry_with_backoff`
   - To: `from infrastructure.tracking.mlflow.utils import retry_with_backoff`

3. ✅ Updated `src/orchestration/jobs/tracking/trackers/conversion_tracker.py`:
   - Changed `from orchestration.jobs.tracking.utils.mlflow_utils import retry_with_backoff`
   - To: `from infrastructure.tracking.mlflow.utils import retry_with_backoff`

4. ✅ Updated `src/infrastructure/platform/adapters/mlflow_context.py`:
   - Changed `from orchestration.jobs.tracking.utils.helpers import create_child_run`
   - To: `from infrastructure.tracking.mlflow.runs import create_child_run`

**Verification**:
- ✅ No remaining imports from `orchestration.jobs.tracking.utils.mlflow_utils` (grep confirmed)
- ✅ No remaining imports from `orchestration.jobs.tracking.utils.helpers` (grep confirmed)
- ✅ No remaining imports from `orchestration.mlflow_utils` (grep confirmed)
- ✅ Remaining `orchestration.*mlflow` imports are for different modules:
  - `orchestration.jobs.tracking.mlflow_types` (domain-specific, not utility re-export)
  - `orchestration.jobs.tracking.mlflow_naming` (domain-specific, not utility re-export)
  - `orchestration.jobs.tracking.mlflow_index` (domain-specific, not utility re-export)
- ✅ Backward compatibility shims in `orchestration.jobs.tracking.utils.*` remain but are unused
- ✅ No linter errors introduced (pre-existing mlflow import warning unrelated)

**Impact**:
- All active code now imports directly from `infrastructure.tracking.mlflow.*`
- Backward compatibility shims remain for safety but are no longer used
- Clear single source of truth established for MLflow utilities
...
2. ✅ Updated module imports verified:
   - `orchestration.jobs.tracking.trackers.benchmark_tracker` imports correctly ✓
   - All tracker modules can be imported without errors ✓

3. ✅ Backward compatibility shims verified:
   - `orchestration.jobs.tracking.utils.mlflow_utils.retry_with_backoff` works ✓
   - `orchestration.jobs.tracking.utils.helpers.create_child_run` works ✓
   - Shims remain functional for safety (though unused by active code)

4. ✅ Linter check:
   - Only pre-existing warning about mlflow package resolution (unrelated to our changes)
   - No import errors introduced

**Verification Results**:
- ✅ All infrastructure imports work correctly
- ✅ All updated tracker files import successfully
- ✅ Backward compatibility shims function correctly
- ✅ No broken imports detected
- ✅ All imports use direct infrastructure paths (no re-export layers in active code)
...
- Full test suite can be run separately if needed (pytest infrastructure may require additional setup)
