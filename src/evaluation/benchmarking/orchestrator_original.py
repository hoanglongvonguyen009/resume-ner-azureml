...
def _find_hpo_experiment_id(
    mlflow_client: Any,
    study_key_hash: Optional[str],
    benchmark_tracker: Optional[Any],
) -> Optional[List[str]]:
    """
    Find HPO experiment ID by study_key_hash.
    
    Args:
        mlflow_client: MLflow client instance
        study_key_hash: Study key hash to search for
        benchmark_tracker: Optional benchmark tracker (for experiment name fallback)
        
    Returns:
        List of experiment IDs, or None if not found
    """
    if not study_key_hash:
        return None
    
    try:
        temp_runs = mlflow_client.search_runs(
            filter_string=f"tags.code.study_key_hash = '{study_key_hash}'",
            max_results=1
        )
        if temp_runs:
            experiment_id = temp_runs[0].info.experiment_id
            logger.info(
...
def _lookup_trial_run_id(
    ...
    """
    Look up trial run ID from MLflow by trial_key_hash.
    
    Args:
        mlflow_client: MLflow client instance
        trial_key_hash: Trial key hash to search for
        study_key_hash: Optional study key hash for experiment filtering
        benchmark_tracker: Optional benchmark tracker
        
    Returns:
        Trial run ID if found, None otherwise
    """
    try:
        experiment_ids_to_search = _find_hpo_experiment_id(
            mlflow_client, study_key_hash, benchmark_tracker
        )
        ...
        if runs:
            ...
            if runs:
                ...
                logger.info(
                    f"[BENCHMARK] Found trial run ID from MLflow: {trial_run_id[:12]}... "
...
def _lookup_refit_run_id(
    ...
    """
    Look up refit run ID from MLflow by trial_key_hash.
    ...
    Returns:
        Refit run ID if found, None otherwise
    """
    try:
        experiment_ids_to_search = _find_hpo_experiment_id(
            mlflow_client, study_key_hash, benchmark_tracker
        ...
        if refit_runs:
            ...
            logger.info(
                f"[BENCHMARK] Found refit run ID from MLflow: {refit_run_id[:12]}..."
            )
            return refit_run_id
...
def _build_benchmark_output_path(
    ...
    trial_id = _extract_trial_id(trial_info)
    study_key_hash = trial_info.get("study_key_hash")
    trial_key_hash = trial_info.get("trial_key_hash")
...
def benchmark_best_trials(
    ...
    for backbone, trial_info in best_trials.items():
        ...
        # Validate trial_info and extract checkpoint_dir
        checkpoint_dir = _validate_trial_info(trial_info, backbone)
        ...
        trial_id = _extract_trial_id(trial_info)
        logger.debug(
            f"[BENCHMARK] Extracted trial_id: {trial_id} from trial_info "
            f"(trial_id={trial_info.get('trial_id')}, trial_name={trial_info.get('trial_name')})"
        )
        
        # Get hashes from trial_info
        study_key_hash = trial_info.get("study_key_hash")
        trial_key_hash = trial_info.get("trial_key_hash")
        
        # Build benchmark output path
        benchmark_output = _build_benchmark_output_path(
        ...
        if not checkpoint_dir.exists():
            logger.warning(
                f"Checkpoint not found for {backbone} {trial_info.get('trial_name', 'unknown')} at {checkpoint_dir}"
            )
            ...
            if "trial_dir" in trial_info and trial_info["trial_dir"]:
                ...
                if trial_dir.exists():
                    try:
                        contents = [item.name for item in trial_dir.iterdir() if item.is_dir()]
                        logger.debug(f"Trial directory {trial_dir} contains: {contents}")
                    except Exception as e:
                        logger.debug(f"Could not list trial directory: {e}")
            continue
        
        # Check if benchmark already exists
        if _check_benchmark_exists(benchmark_output, restore_from_drive):
            logger.info(
                f"Benchmark results already exist - skipping benchmarking for {backbone}"
            )
            benchmark_results[backbone] = benchmark_output
            continue
        
        logger.info(f"Benchmarking {backbone} ({trial_info.get('trial_name', 'unknown')})...")
        
        # Resolve run IDs (from trial_info or MLflow lookup)
        hpo_trial_run_id, hpo_refit_run_id, hpo_sweep_run_id = _resolve_run_ids(
            trial_info=trial_info,
        ...
        # Build benchmark_key for idempotency
        benchmark_key = _build_benchmark_key_for_trial(
            trial_info=trial_info,
            benchmark_config=benchmark_config,
            data_config=data_config,
            hpo_config=hpo_config,
        ...
        # Compute benchmark_config_hash for tracking
        benchmark_config_hash = None
        if benchmark_config:
            try:
                from common.shared.hash_utils import compute_json_hash
