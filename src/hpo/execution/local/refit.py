...
def run_refit_training(
    ...
    # Create refit run as child of HPO parent
    refit_run_id = _create_refit_mlflow_run(
        hpo_parent_run_id=hpo_parent_run_id,
        refit_tags=refit_tags,
        refit_run_name=refit_run_name,
    )
    ...
    if result.returncode != 0:
        logger.error(
            f"[REFIT] Refit training failed with return code {result.returncode}")
        logger.error(f"[REFIT] STDOUT:\n{result.stdout}")
        logger.error(f"[REFIT] STDERR:\n{result.stderr}")
        raise RuntimeError(f"Refit training failed: {result.stderr}")
...
def _find_project_root(config_dir: Path) -> Path:
    ...
    while depth < max_depth:
        if (candidate_root / "src").exists() and (candidate_root / "src" / "training").exists():
            logger.info(
                f"[REFIT] Found project root: {candidate_root} (from config_dir: {config_dir})"
            )
    ...
    logger.warning(
        f"[REFIT] Could not find project root with src/training/ directory after {max_depth} levels. "
        f"Using {root_dir} as root_dir. Config_dir: {config_dir}"
    )
    return root_dir


def _build_refit_command(
    dataset_path: str,
    config_dir: Path,
    backbone: str,
    ...
    args = [
        ...
        "--backbone",
        backbone,
        "--use-all-data", "true",  # Refit uses full training set, no validation split
    ]

    # Add hyperparameters from best trial
    if "learning_rate" in refit_params:
        args.extend(["--learning-rate", str(refit_params["learning_rate"])])
    if "batch_size" in refit_params:
        args.extend(["--batch-size", str(refit_params["batch_size"])])
    if "dropout" in refit_params:
        args.extend(["--dropout", str(refit_params["dropout"])])
    if "weight_decay" in refit_params:
        args.extend(["--weight-decay", str(refit_params["weight_decay"])])

    # Use full epochs for refit (not minimal like HPO)
    epochs = train_config.get("training", {}).get("epochs", 10)
    args.extend(["--epochs", str(epochs)])

    # Enable early stopping for refit
    args.extend(["--early-stopping-enabled", "true"])

    return args


def _setup_refit_environment(
    refit_output_dir: Path,
    root_dir: Path,
    mlflow_experiment_name: str,
) -> Dict[str, str]:
    """Set up environment variables for refit training subprocess."""
    env = os.environ.copy()
    env["AZURE_ML_OUTPUT_CHECKPOINT"] = str(refit_output_dir)
    env["AZURE_ML_OUTPUT_checkpoint"] = str(refit_output_dir)
...
def _create_refit_mlflow_run(
    hpo_parent_run_id: Optional[str],
    refit_tags: Dict[str, str],
    refit_run_name: str,
) -> Optional[str]:
    """Create MLflow run for refit training."""
    if not hpo_parent_run_id:
        return None

    try:
        client = mlflow.tracking.MlflowClient()
        parent_run_info = client.get_run(hpo_parent_run_id)
        ...
        # Log refit run URL
        try:
            refit_url = get_mlflow_run_url(experiment_id, refit_run_id)
            logger.info(f"🏃 View refit run {refit_run_name} at: {refit_url}")
        except Exception as url_error:
            logger.debug(f"Could not construct refit run URL: {url_error}")

        logger.info(
            f"[REFIT] Created refit run: {refit_run_id[:12]}... "
            f"as child of HPO parent {hpo_parent_run_id[:12]}..."
        )
        return refit_run_id
    except Exception as e:
...
def _verify_refit_environment(root_dir: Path, env: Dict[str, str]) -> None:
    """Verify environment is set up correctly for refit training."""
    src_dir = str((root_dir / "src").resolve())
    training_module_path = root_dir / "src" / "training" / "__init__.py"

    if not training_module_path.exists():
        raise RuntimeError(
            f"[REFIT] Training module not found at {training_module_path}. "
    ...
    if not pythonpath_value:
        raise RuntimeError(
            f"[REFIT] PYTHONPATH is not set in environment! "
            f"This will prevent the subprocess from finding the training module."
        )
    if src_dir not in pythonpath_value:
        raise RuntimeError(
            f"[REFIT] src_dir ({src_dir}) is not in PYTHONPATH ({pythonpath_value})! "
            f"This will prevent the subprocess from finding the training module."
        )
