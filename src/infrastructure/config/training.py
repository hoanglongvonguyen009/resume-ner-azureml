...
def _compute_fingerprints(
    ...
    """
    try:
        from infrastructure.fingerprints import compute_spec_fp, compute_exec_fp
        ...
        include_precision_fp = identity_config.get(
            "include_precision_fp", True)
        include_determinism_fp = identity_config.get(
        ...
        # Use short placeholders that make sense when truncated to 8 chars for naming
        warnings.warn(
            "Fingerprint computation functions not available. Using placeholder fingerprints.",
            RuntimeWarning,
            stacklevel=2
        )
        # Use "unknown" which is 7 chars - will be used as-is (not truncated)
...
def _resolve_checkpoint_from_best_config(
    ...
    if trial_id:
        ...
        old_checkpoint = root_dir / "outputs" / "hpo" / \
            backbone_name / trial_id / "checkpoint"
        if validate_checkpoint(old_checkpoint):
            return old_checkpoint

        # Try benchmarking path structure
        bench_checkpoint = root_dir / "outputs" / "benchmarking" / \
            backbone_name / trial_id / "checkpoint"
...
def _resolve_checkpoint_from_fingerprints(
    ...
    try:
        from infrastructure.metadata.training import find_by_spec_and_env, get_latest_entry
...
def _find_existing_variant(
    ...
    try:
        ...
        if entries:
            # Get highest variant
            variants = [e.get("variant", 1)
                        for e in entries if e.get("exec_fp") == exec_fp]
            if variants:
                return max(variants)
    except ImportError:
        pass
    ...
    try:
        ...
        if metadata_file.exists():
            try:
                ...
                metadata = load_json(metadata_file, default={})
                if metadata.get("status", {}).get("training", {}).get(
                    "completed", False
                ):
                    return True
            except Exception:
        ...
        # Fallback: check if checkpoint exists and has model files
        if checkpoint_dir.exists():
            ...
            if config_file.exists() and (
                model_files
                or any((checkpoint_dir / f).exists() for f in required_files)
            ):
                return True
