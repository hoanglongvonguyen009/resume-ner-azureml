...
def create_mlflow_sweep_run(
    ...
    CRITICAL: Does NOT use context manager - run stays active for caller to manage.
    This allows log_sweep_metadata() to use mlflow.set_tag() without starting a new run.
    
    Returns:
        Tuple of (RunHandle, study_key_hash, study_family_hash)
    """
    # CRITICAL: Don't use context manager - run must stay active for log_sweep_metadata()
    # The context manager would exit when this function returns, causing mlflow.set_tag()
    # in log_sweep_metadata() to try to start a new run without a name.
