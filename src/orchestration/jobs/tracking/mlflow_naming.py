...
def build_mlflow_run_key(context: NamingContext) -> str:
    ...
    if context.process_type == "hpo":
        ...
        if not context.spec_fp or not context.exec_fp:
            raise ValueError("Final training requires spec_fp and exec_fp for run_key")
        ...
        if not context.parent_training_id or not context.conv_fp:
            raise ValueError("Conversion requires parent_training_id and conv_fp for run_key")
...
def build_mlflow_run_name(
    ...
    Keeps names short and readable while maintaining some context.
    Supports auto-increment versioning for HPO and benchmarking when enabled.
    
    Args:
        context: NamingContext with all required information.
        config_dir: Optional config directory for reading run_name settings.
        root_dir: Optional project root directory (inferred from output_dir if not provided).
        output_dir: Optional output directory (used to infer root_dir if not provided).
    ...
    if root_dir is None and output_dir is not None:
        # output_dir is typically outputs/{stage}/..., so root_dir is output_dir.parent.parent
    ...
    # Get config for run_name settings
    ...
    if context.process_type == "hpo":
        ...
        # For parent runs, trial_id is the study_name (e.g., "hpo_distilbert_smoke_test_3_24")
        # For trial runs, trial_id is like "trial_0_20251231_005"
        if context.trial_id and context.trial_id.startswith("hpo_"):
            # Parent run: preserve original study_name, append auto-incremented version
            base_without_env = _strip_env_prefix(context.trial_id, context.environment)
            ...
            if auto_inc_config.get("enabled") and auto_inc_config.get("processes", {}).get("hpo"):
                try:
                    ...
                    # Append version with underscore to preserve original study_name version
                    return f"{env_prefix}{base_without_env}_{version}"
                except Exception as e:
                    # If reservation fails, fall back to current behavior
                    import logging
                    logger = logging.getLogger(__name__)
                    logger.warning(f"Could not reserve version for HPO run name: {e}, using fallback")
                    return f"{env_prefix}{context.trial_id}"
            else:
                # Disabled: use study_name as-is (current behavior)
                return f"{env_prefix}{context.trial_id}"
        else:
            # Trial run: build name with model and trial_id
        ...
        if auto_inc_config.get("enabled") and auto_inc_config.get("processes", {}).get("benchmarking"):
            try:
                ...
                # Preserve trial_id information (contains trial number and timestamp from HPO)
                if context.trial_id:
                    return f"benchmark_{context.model}_{context.trial_id}_{version}"
                else:
                    # Fallback if no trial_id
                    return f"{base_name}_{version}"
            except Exception as e:
                # If reservation fails, fall back to current behavior
                import logging
                logger = logging.getLogger(__name__)
                logger.warning(f"Could not reserve version for benchmarking run name: {e}, using fallback")
                trial_short = context.trial_id[:20] if context.trial_id and len(context.trial_id) > 20 else (context.trial_id or "unknown")
        ...
        # Shortened: final_training_distilbert_v1_specabc_exexyz
        if shorten_fingerprints:
            spec_short = (context.spec_fp or "unknown")[:8] if context.spec_fp else "unknown"
            exec_short = (context.exec_fp or "unknown")[:8] if context.exec_fp else "unknown"
        else:
            spec_short = context.spec_fp or "unknown"
            exec_short = context.exec_fp or "unknown"
        return f"final_training_{context.model}_v{context.variant}_{spec_short}_{exec_short}"
    
    elif context.process_type == "conversion":
        # Shortened conversion name
        parent_short = context.parent_training_id[:30] if context.parent_training_id and len(context.parent_training_id) > 30 else (context.parent_training_id or "unknown")
        if shorten_fingerprints:
            conv_short = (context.conv_fp or "unknown")[:8] if context.conv_fp else "unknown"
        else:
            conv_short = context.conv_fp or "unknown"
        return f"conversion_{context.model}_{parent_short}_{conv_short}"
...
def build_mlflow_tags(
    ...
    if context:
        if sanitize_tags:
            tags["code.stage"] = sanitize_tag_value(context.process_type, max_length=tag_max_length, config_dir=config_dir)
            tags["code.model"] = sanitize_tag_value(context.model, max_length=tag_max_length, config_dir=config_dir)
            tags["code.env"] = sanitize_tag_value(context.environment, max_length=tag_max_length, config_dir=config_dir)
        else:
        ...
        if sanitize_tags:
            ...
            tags["code.env"] = sanitize_tag_value(env, max_length=tag_max_length, config_dir=config_dir)
        else:
            tags["code.stage"] = "unknown"
            tags["code.model"] = "unknown"
            tags["code.env"] = env
    ...
    if sanitize_tags:
        tags["code.created_by"] = sanitize_tag_value(created_by, max_length=tag_max_length, config_dir=config_dir)
    else:
    ...
    if sanitize_tags:
        tags["code.project"] = sanitize_tag_value(project_name, max_length=tag_max_length, config_dir=config_dir)
    else:
    ...
    if context:
        ...
        try:
            ...
            if len(run_key) <= 200:
                if sanitize_tags:
                    tags["code.run_key"] = sanitize_tag_value(run_key, max_length=tag_max_length, config_dir=config_dir)
                else:
            ...
            if context.spec_fp:
                if sanitize_tags:
                    tags["code.spec_fp"] = sanitize_tag_value(context.spec_fp, max_length=tag_max_length, config_dir=config_dir)
                else:
                    tags["code.spec_fp"] = context.spec_fp
            if context.exec_fp:
                if sanitize_tags:
                    tags["code.exec_fp"] = sanitize_tag_value(context.exec_fp, max_length=tag_max_length, config_dir=config_dir)
                else:
                    tags["code.exec_fp"] = context.exec_fp
            if context.variant:
                if sanitize_tags:
                    tags["code.variant"] = sanitize_tag_value(str(context.variant), max_length=tag_max_length, config_dir=config_dir)
                else:
                    tags["code.variant"] = str(context.variant)
            if context.trial_id:
                if sanitize_tags:
                    tags["code.trial_id"] = sanitize_tag_value(context.trial_id, max_length=tag_max_length, config_dir=config_dir)
                else:
                    tags["code.trial_id"] = context.trial_id
            if context.parent_training_id:
                if sanitize_tags:
                    tags["code.parent_training_id"] = sanitize_tag_value(context.parent_training_id, max_length=tag_max_length, config_dir=config_dir)
                else:
                    tags["code.parent_training_id"] = context.parent_training_id
            if context.conv_fp:
                if sanitize_tags:
                    tags["code.conv_fp"] = sanitize_tag_value(context.conv_fp, max_length=tag_max_length, config_dir=config_dir)
                else:
    ...
    if output_dir:
        if sanitize_tags:
            tags["code.output_dir"] = sanitize_tag_value(str(output_dir), max_length=tag_max_length, config_dir=config_dir)
        else:
            tags["code.output_dir"] = str(output_dir)
    
    if parent_run_id:
        if sanitize_tags:
            tags["code.parent_run_id"] = sanitize_tag_value(parent_run_id, max_length=tag_max_length, config_dir=config_dir)
        else:
            tags["code.parent_run_id"] = parent_run_id
    
    if group_id:
        if sanitize_tags:
            tags["code.group_id"] = sanitize_tag_value(group_id, max_length=tag_max_length, config_dir=config_dir)
        else:
