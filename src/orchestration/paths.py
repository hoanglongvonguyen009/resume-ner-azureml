...
def get_cache_file_path(
    ...
    """
    paths_config = load_paths_config(config_dir)
    cache_dir = resolve_output_path(root_dir, config_dir, "cache", subcategory=cache_type)
    
    if filename:
        return cache_dir / filename
    
    # Get strategy config
    strategy_config = paths_config.get("cache_strategies", {}).get(cache_type, {})
...
def save_cache_with_dual_strategy(
    ...
    latest_file = get_cache_file_path(root_dir, config_dir, cache_type, file_type="latest")
    index_file = get_cache_file_path(root_dir, config_dir, cache_type, file_type="index")
    ...
    if strategy_config.get("index", {}).get("enabled", True):
        ...
        if strategy_config.get("index", {}).get("include_metadata", True):
            ...
            if "selection_criteria" in data:
                index_entry["best_value"] = data.get("selection_criteria", {}).get("best_value")
            if "metrics" in data:
                index_entry["metrics"] = {
