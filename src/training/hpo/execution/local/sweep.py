...
from training.hpo.core.search_space import translate_search_space_to_optuna
                        ...
                        try:
                            ...
                            if upload_succeeded:
                                ...
                                if result:
                                    ...
                                    logger.warning(
                                        f"[REFIT] Failed to terminate refit run {refit_run_id[:12]}... (terminate_run_with_tags returned False)")
                            else:
                                ...
                                if result:
                                    ...
                                    logger.warning(
                                        f"[REFIT] Failed to mark refit run as FAILED: {refit_run_id[:12]}... (terminate_run_with_tags returned False)")
                        except Exception as term_error:
                            logger.error(
                                f"[REFIT] Error terminating refit run {refit_run_id[:12] if refit_run_id else 'unknown'}...: {term_error}",
                                exc_info=True
                            )
